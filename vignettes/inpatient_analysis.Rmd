...
Figure \@ref(fig:icd10-dist-plot) shows the distribution of primary and secondary heart failure ICD10 codes. In some cases, there are encounters with multiple heart failure primary diagnosis, (n = **`r nrow(hf_diag_primary_count)`**) and similarly with secondary diagnosis (n = **`r nrow(hf_diag_secondary_count)`**). 
...
tab_cap <- paste(
  ...
  dplyr::n_distinct(primary_diag_of_secondary$PERSON_ID),
  "patients).",
  sep=" "
)
...
p1 <- proportion_bar_plot(primary_enc,"ADMIT_MODE_CD","Distribution of Mode of Admission",n_suffix="encounters",vjust=0.25) + ylim(0,800) +xlab("")  + coord_flip()

p2 <- proportion_bar_plot(primary_enc,"DISCH_DISPOSITION_CD","Distribution of Mode of Separation",n_suffix="encounters",vjust=0.25)  + ylim(0,1100) +xlab("")  + coord_flip()
...
plot_histogram(
  ...
  median(primary_enc_alert$ALERT_DELTA,na.rm=TRUE),n_suffix = "patients") + xlim(-100,100) + xlab("Time (days)")
```
```{r}
...
Figure \@ref(fig:form-plot) shows the proportion of patients who have had a referral form or enrolment form completed in the EMR. Figure \@ref(fig:no-form-disch-plot) shows the discharge modes for patients without either a referral nor an enrolment form (n = **`r sum(form_primary_cohort$NO_FORM)`**).
