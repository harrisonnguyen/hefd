...
Figure \@ref(fig:alert-by-age-plot) shows a boxplot of `AGE` against whether the patient has had an ACD or not (`HAS_ALERT`). The plot suggests that the means the `AGE` of the group with an ACD is higher compared to the group without an ACD (**`r round(mean[2,2],1)`** vs **`r round(mean[1,2],1)`**, respectively). Furthermore the group without an ACD has larger variance w.r.t `AGE`. 
...
Figure \@ref(fig:order-year-plot) shows the proportion of encounters with an order during the encounter OR within one last year of the beginning of the encounter's admission. These proportions for all of these orders is slightly higher compared to Figure \@ref(fig:order-prop-plot).

```{r order-year-plot,fig.cap="Proportion of encounters with an order during the encounter OR within the last year of admission."}
...
Figure \@ref(fig:form-by-month-plot) shows the proportion of encounters with either an enrolment, referral form or neither by periods of 6 months. Figure \@ref(fig:form-by-month-plot)**B** shows that the proportion with enrolment forms is increasing over the **`r time_period`** year period.
