...
```{r alert-by-age-plot,fig.cap="Boxplot of Age and whether patient has had a ACD or not."}
...
Figure \@ref(fig:alert-by-age-plot) shows a boxplot of `AGE` against whether the patient has had an ACD or not (`HAS_ALERT`). The plot suggests that the means the `AGE` of the group with an ACD is higher compared to the group without an ACD (**`r mean[2,2]`** vs **`r mean[2,1]`**, respectively). Furthermore the group without an ACD has larger variance w.r.t `AGE`. 
...
```{r age-alert-plot,"Likelihood of having an ACD given the patient's age."}
...
primary_orders_in_year<-hf_order %>%
  dplyr::mutate(HAS_ECHO_YEAR = ENCNTR_ID %in% echo_in_year_temp$ENCNTR_ID,
...
```{r}
plot_logical_columns(primary_orders_in_year,"ORDERS","Proportion of Encounters with Order within 1 Year",n_suffix="encounters")
```
...
```{r}
extract_form_with_year_quarter <- function(df,string,prefix){
  df %<>% dplyr::filter(DESCRIPTION == string) %>%
  dplyr::mutate(YEAR_QUARTER = zoo::as.yearqtr(FORM_DT_TM),
                HAS_FORM=TRUE) %>%
  dplyr::select(PERSON_ID,ENCNTR_ID,PARENT_ENTITY_ID,FORM_DT_TM,YEAR_QUARTER,HAS_FORM) %>%
  dplyr::group_by(PERSON_ID,YEAR_QUARTER) %>%
  dplyr::arrange(desc(FORM_DT_TM)) %>%
  dplyr::slice(1) %>%
    dplyr::rename_with(.fn = ~ paste0(prefix,"_",.x))
...
enrolment_form <- hf_form %>%
  extract_form_with_year_quarter('Heart Failure - Management of Cardiac Function Enrolment',"ENROLMENT")


referral_form <- hf_form %>%
   extract_form_with_year_quarter('Heart Failure (MACARF) Referral Form',"REFERRAL")
  
 

temp <- primary_enc %>%
  dplyr::mutate(YEAR_QUARTER = zoo::as.yearqtr(DISCH_DT_TM)) %>%
  dplyr::left_join(enrolment_form,by=c("PERSON_ID" = "ENROLMENT_PERSON_ID",
                                       "YEAR_QUARTER" = "ENROLMENT_YEAR_QUARTER")) %>%
  dplyr::left_join(referral_form,by=c("PERSON_ID"= "REFERRAL_PERSON_ID",
                                       "YEAR_QUARTER" = "REFERRAL_YEAR_QUARTER")) %>%
   tidyr::replace_na(list(ENROLMENT_HAS_FORM = FALSE, REFERRAL_HAS_FORM = FALSE)) %>%
  dplyr::mutate(NO_FORM = !ENROLMENT_HAS_FORM & !REFERRAL_HAS_FORM) %>%
  dplyr::mutate(YEAR_QUARTER = as.character(YEAR_QUARTER))

temp %>%
conditional_bar_plot("YEAR_QUARTER","REFERRAL_HAS_FORM","Referral form by Quarter") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + xlab("")


temp %>%
conditional_bar_plot("YEAR_QUARTER","ENROLMENT_HAS_FORM","Enrolment form by Quarter") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + xlab("")

temp %>%
conditional_bar_plot("YEAR_QUARTER","NO_FORM","No form by Quarter") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + xlab("")
```
